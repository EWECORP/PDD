...
def main():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            for key, query in {
                'classification_types': 'select id,value from inventory.inv_product_classification_type order by value',
                'classification_distribution': """select t.value as classification_type,c.classification_value,count(*) as rows,count(*) filter(where c.active) as active_rows from inventory.inv_product_classification c join inventory.inv_product_classification_type t on t.id=c.classification_type_id group by 1,2 order by 1,2""",
                'classification_quality': """select count(*) as rows,count(distinct product_id) as products,count(*) filter(where active) as active_rows,count(*) filter(where valid_to<valid_from) as inverted_dates,count(*) filter(where btrim(classification_value)='') as empty_values,count(*) filter(where active and (valid_from is null or valid_from<=current_date) and (valid_to is null or current_date<valid_to)) as effective_today from inventory.inv_product_classification""",
                'classification_duplicates': """select count(*) as duplicated_product_types from (select product_id,classification_type_id from inventory.inv_product_classification where active group by 1,2 having count(*)>1) x""",
                'classification_sample': """select p.ext_code,t.value as classification_type,c.classification_value,c.active,c.valid_from,c.valid_to from inventory.inv_product_classification c join inventory.inv_product_classification_type t on t.id=c.classification_type_id join inventory.inv_product p on p.id=c.product_id order by p.ext_code,t.value limit 25""",
                'snapshot_function': """select p.proname,p.prosrc from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='supply_planning' and p.proname='spl_forecast_planning_input_immutable'""",
                'snapshot_trigger': """select tgname,tgenabled,pg_get_triggerdef(oid) as definition from pg_trigger where tgrelid=to_regclass('supply_planning.spl_forecast_planning_input') and not tgisinternal""",
            }.items():
                cur.execute(query);result['checks'][key]=[dict(r) for r in cur.fetchall()]
        dest=Path(__file__).with_name('verificacion_clasificaciones_20260917.json')
