...
ALTER TABLE stock_management.pdd_pdvb_backtest_metric
...
UPDATE stock_management.pdd_pdvb_backtest_metric
...
ALTER TABLE stock_management.pdd_pdvb_backtest_metric
    ...
    DROP CONSTRAINT uq_pdd_pdvb_backtest_metric;

ALTER TABLE stock_management.pdd_pdvb_backtest_metric
    ADD CONSTRAINT ck_pdd_pdvb_backtest_estimator CHECK (
        estimator_code IN (
            'PDVB_CANDIDATE', 'MEAN_28',
            'ALGO_01_GROWTH', 'ALGO_01_NORMALIZED'
        )
    ),
    ADD CONSTRAINT ck_pdd_pdvb_backtest_sample CHECK (
        sample_code IN ('OWN_VALID', 'COMMON_VALID')
    ),
    ADD CONSTRAINT ck_pdd_pdvb_backtest_coverage_counts CHECK (
        expected_count > 0
    ...
    ADD CONSTRAINT uq_pdd_pdvb_backtest_metric UNIQUE (
...
COMMENT ON COLUMN stock_management.pdd_pdvb_backtest_metric.estimator_code IS
'Identifica el modelo candidato o benchmark evaluado sobre la misma corrida.';

COMMENT ON COLUMN stock_management.pdd_pdvb_backtest_metric.sample_code IS
