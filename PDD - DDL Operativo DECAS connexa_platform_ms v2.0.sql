-- PDD / Posiciones, necesidades DECAS, backlog e integracion - v2.0 - 2026-08-03
-- Base de datos objetivo: connexa_platform_ms
-- Esquema objetivo: pdd
-- Requiere "PDD - DDL Operativo Core connexa_platform_ms v2.0.sql".
...
BEGIN
    IF current_database() <> 'connexa_platform_ms' THEN
        RAISE EXCEPTION
            'DDL PDD DECAS: base incorrecta (%). Se esperaba connexa_platform_ms.',
            current_database();
    END IF;
END
$guard$;

CREATE TABLE pdd.item_logistics_snapshot (
    ...
    source_snapshot_id bigint
        REFERENCES pdd.source_snapshot(source_snapshot_id) ON DELETE RESTRICT,
...
CREATE TABLE pdd.branch_stock_position (
    ...
    configuration_version_id bigint NOT NULL
        REFERENCES pdd.configuration_version(configuration_version_id) ON DELETE RESTRICT,
...
CREATE INDEX ix_branch_stock_position_pair_date
    ON pdd.branch_stock_position (codigo_articulo, sucursal, business_date DESC);

CREATE INDEX ix_branch_stock_position_run_status
    ON pdd.branch_stock_position (calculation_run_id, calculation_status);

CREATE INDEX ix_branch_stock_position_date_brin
    ON pdd.branch_stock_position USING brin (business_date);

CREATE TABLE pdd.cd_stock_position (
    business_date date NOT NULL,
...
CREATE INDEX ix_cd_stock_position_article_date
    ON pdd.cd_stock_position (codigo_articulo, business_date DESC);

CREATE INDEX ix_cd_stock_position_run_status
    ON pdd.cd_stock_position (calculation_run_id, status);

CREATE TABLE pdd.need_snapshot (
    business_date date NOT NULL,
    ...
    logistics_snapshot_id bigint
        REFERENCES pdd.item_logistics_snapshot(item_logistics_snapshot_id) ON DELETE RESTRICT,
    ...
    FOREIGN KEY (business_date, branch_stock_position_id)
        REFERENCES pdd.branch_stock_position (business_date, branch_stock_position_id)
        ON DELETE RESTRICT,
    FOREIGN KEY (scope_version_id, sucursal, codigo_articulo)
        REFERENCES pdd.distribution_scope_pair
            (scope_version_id, destination_branch, codigo_articulo)
...
CREATE INDEX ix_need_snapshot_pair_date
    ON pdd.need_snapshot (codigo_articulo, sucursal, business_date DESC, need_type);

CREATE TABLE pdd.directed_need (
...
CREATE INDEX ix_directed_need_active
    ON pdd.directed_need (need_type, valid_from, valid_to, priority_score DESC)
    WHERE status = 'ACTIVE';

CREATE TABLE pdd.directed_need_line (
    directed_need_line_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    directed_need_id bigint NOT NULL
        REFERENCES pdd.directed_need(directed_need_id) ON DELETE RESTRICT,
    sucursal integer NOT NULL,
    codigo_articulo integer NOT NULL,
...
CREATE INDEX ix_directed_need_line_open
    ON pdd.directed_need_line (sucursal, codigo_articulo, target_date)
    WHERE open_quantity > 0;

CREATE TABLE pdd.directed_need_version (
...
CREATE TABLE pdd.current_backlog_line (
...
CREATE TABLE pdd.backlog_source_allocation (
    backlog_source_allocation_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    backlog_line_id bigint NOT NULL
        REFERENCES pdd.current_backlog_line(backlog_line_id) ON DELETE CASCADE,
...
CREATE INDEX ix_valkimia_import_status_date
    ON pdd.valkimia_import (status, requested_at DESC);

CREATE TABLE pdd.valkimia_import_line (
    valkimia_import_line_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    valkimia_import_id bigint NOT NULL
        REFERENCES pdd.valkimia_import(valkimia_import_id) ON DELETE RESTRICT,
...
CREATE INDEX ix_valkimia_import_line_external
    ON pdd.valkimia_import_line (external_reference, external_line_reference);

CREATE TABLE pdd.execution_event (
    ...
    valkimia_import_line_id bigint NOT NULL
        REFERENCES pdd.valkimia_import_line(valkimia_import_line_id) ON DELETE RESTRICT,
...
CREATE INDEX ix_execution_event_line_date
    ON pdd.execution_event (valkimia_import_line_id, external_occurred_at);

CREATE INDEX ix_execution_event_document
    ON pdd.execution_event (external_document, external_line_reference);

CREATE TABLE pdd.integration_message (
...
CREATE INDEX ix_integration_message_work
    ON pdd.integration_message (status, next_attempt_at, created_at)
    WHERE status IN ('PENDING', 'RETRY');

CREATE INDEX ix_integration_message_correlation
    ON pdd.integration_message (correlation_id, created_at);

CREATE TABLE pdd.business_event_log (
