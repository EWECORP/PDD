-- PDD / Dominio operativo, gobierno y publicacion PDVB - v2.2 - 2026-08-05
-- Bases admitidas: connexa_platform_test (Test) / connexa_platform_ms (Produccion)
-- Esquema objetivo: stock_management
-- Aplicar antes de "PDD - DDL Operativo DECAS connexa_platform_ms v2.2.sql".
...
BEGIN
    IF current_database() NOT IN ('connexa_platform_test', 'connexa_platform_ms') THEN
        RAISE EXCEPTION
            'DDL PDD operativo: base incorrecta (%). Se esperaba connexa_platform_test o connexa_platform_ms.',
            current_database();
    END IF;
END
$guard$;
...
CREATE TABLE stock_management.pdd_configuration_version (
    ...
    CONSTRAINT uq_pdd_configuration_version_uuid UNIQUE (configuration_version_uuid),
    CONSTRAINT uq_pdd_configuration_version_code UNIQUE (configuration_code, version_no),
    CONSTRAINT ck_pdd_configuration_version_validity CHECK (
        valid_to IS NULL OR valid_from IS NULL OR valid_to >= valid_from
    ),
    CONSTRAINT ck_pdd_configuration_version_approval CHECK (
        status <> 'APPROVED' OR (approved_at IS NOT NULL AND approved_by IS NOT NULL)
    )
);

CREATE TABLE stock_management.pdd_pdvb_model_version (
    ...
    approved_at timestamptz,
    approved_by varchar(100),
    CONSTRAINT uq_pdd_pdvb_model_version_uuid UNIQUE (model_version_uuid),
    CONSTRAINT uq_pdd_pdvb_model_version UNIQUE (model_code, version_no),
    CONSTRAINT ck_pdd_pdvb_model_validity CHECK (
        valid_to IS NULL OR valid_from IS NULL OR valid_to >= valid_from
    ),
    CONSTRAINT ck_pdd_pdvb_model_approval CHECK (
        status <> 'APPROVED' OR (approved_at IS NOT NULL AND approved_by IS NOT NULL)
    )
);

CREATE TABLE stock_management.pdd_distribution_scope_version (
    ...
    CONSTRAINT uq_pdd_distribution_scope_version_uuid UNIQUE (scope_version_uuid),
    CONSTRAINT uq_pdd_distribution_scope_version UNIQUE (origin_cd, business_date, checksum),
    CONSTRAINT ck_pdd_distribution_scope_phase1_origin CHECK (origin_cd = 41),
    CONSTRAINT ck_pdd_distribution_scope_current CHECK (NOT is_current OR status = 'APPROVED'),
    CONSTRAINT ck_pdd_distribution_scope_approval CHECK (
        status <> 'APPROVED' OR (approved_at IS NOT NULL AND approved_by IS NOT NULL)
    )
);

CREATE UNIQUE INDEX uq_pdd_distribution_scope_current
    ON stock_management.pdd_distribution_scope_version (origin_cd)
    WHERE is_current;

CREATE TABLE stock_management.pdd_distribution_scope_article (
    ...
    CONSTRAINT ck_pdd_distribution_scope_article_purchase CHECK (cd_active_for_purchase)
);

CREATE INDEX ix_pdd_distribution_scope_article_lookup
    ON stock_management.pdd_distribution_scope_article (codigo_articulo, scope_version_id DESC);

CREATE TABLE stock_management.pdd_distribution_scope_pair (
    scope_version_id bigint NOT NULL,
    origin_cd integer NOT NULL,
    ...
    FOREIGN KEY (scope_version_id, codigo_articulo)
        REFERENCES stock_management.pdd_distribution_scope_article (scope_version_id, codigo_articulo)
        ON DELETE RESTRICT,
    CONSTRAINT ck_pdd_distribution_scope_distinct_branch CHECK (destination_branch <> origin_cd),
    CONSTRAINT ck_pdd_distribution_scope_pair_phase1_cd41 CHECK (
        origin_cd = 41
        AND route_code = '41CD'
        AND supply_mode = 0
...
CREATE INDEX ix_pdd_distribution_scope_pair_operational
    ON stock_management.pdd_distribution_scope_pair
    (scope_version_id, destination_branch, codigo_articulo, c_proveedor_primario);

CREATE TABLE stock_management.pdd_calculation_run (
    ...
    scope_version_id bigint REFERENCES stock_management.pdd_distribution_scope_version(scope_version_id),
    model_version_id bigint REFERENCES stock_management.pdd_pdvb_model_version(model_version_id),
    configuration_version_id bigint
        REFERENCES stock_management.pdd_configuration_version(configuration_version_id),
    ...
    error_detail text,
    CONSTRAINT uq_pdd_calculation_run_uuid UNIQUE (calculation_run_uuid),
    CONSTRAINT uq_pdd_calculation_run_attempt UNIQUE (
        run_type, business_date, scope_type, scope_id, attempt_no
    ),
    CONSTRAINT ck_pdd_calculation_cutoff CHECK (cutoff_date < business_date),
    CONSTRAINT ck_pdd_calculation_times CHECK (
        finished_at IS NULL OR started_at IS NULL OR finished_at >= started_at
    ),
    CONSTRAINT ck_pdd_calculation_current_success CHECK (
        NOT is_current OR status = 'SUCCEEDED'
    ),
    CONSTRAINT ck_pdd_calculation_scope_required CHECK (
        run_type NOT IN ('PDVB', 'DAILY_DECAS') OR scope_version_id IS NOT NULL
    ),
    CONSTRAINT ck_pdd_calculation_model_required CHECK (
        run_type NOT IN ('PDVB', 'BACKTEST') OR model_version_id IS NOT NULL
    ),
    CONSTRAINT ck_pdd_calculation_config_required CHECK (
        run_type <> 'DAILY_DECAS' OR configuration_version_id IS NOT NULL
    )
);

CREATE UNIQUE INDEX uq_pdd_calculation_run_current
    ON stock_management.pdd_calculation_run (run_type, business_date, scope_type, scope_id)
    WHERE is_current;

CREATE INDEX ix_pdd_calculation_run_status_date
    ON stock_management.pdd_calculation_run (status, business_date DESC);

CREATE TABLE stock_management.pdd_source_snapshot (
    ...
    detail jsonb NOT NULL DEFAULT '{}'::jsonb,
    error_detail text,
    CONSTRAINT uq_pdd_source_snapshot_run UNIQUE (calculation_run_id, source_code),
    CONSTRAINT ck_pdd_source_snapshot_range CHECK (
...
CREATE INDEX ix_pdd_source_snapshot_status
    ON stock_management.pdd_source_snapshot (calculation_run_id, status)
    WHERE status <> 'VALID';

CREATE TABLE stock_management.pdd_pdvb_publication_batch (
    ...
    error_detail text,
    detail jsonb NOT NULL DEFAULT '{}'::jsonb,
    CONSTRAINT uq_pdd_pdvb_publication_batch_uuid UNIQUE (publication_batch_uuid),
    CONSTRAINT uq_pdd_pdvb_publication_run UNIQUE (calculation_run_id),
    CONSTRAINT ck_pdd_pdvb_publication_validation CHECK (
    ...
    CONSTRAINT ck_pdd_pdvb_publication_published CHECK (
        status <> 'PUBLISHED'
        OR (published_at IS NOT NULL AND published_row_count = expected_row_count)
    )
);

CREATE TABLE stock_management.pdd_pdvb_publication_stage (
    publication_batch_id bigint NOT NULL
        REFERENCES stock_management.pdd_pdvb_publication_batch(publication_batch_id) ON DELETE CASCADE,
    business_date date NOT NULL,
    analytical_detail_id bigint NOT NULL,
    model_version_id bigint NOT NULL
        REFERENCES stock_management.pdd_pdvb_model_version(model_version_id) ON DELETE RESTRICT,
    scope_version_id bigint NOT NULL
        REFERENCES stock_management.pdd_distribution_scope_version(scope_version_id) ON DELETE RESTRICT,
    origin_cd integer NOT NULL,
    codigo_articulo integer NOT NULL,
    ...
    FOREIGN KEY (scope_version_id, sucursal, codigo_articulo)
        ...
        ON DELETE RESTRICT,
    CONSTRAINT ck_pdd_pdvb_stage_origin CHECK (origin_cd = 41),
    CONSTRAINT ck_pdd_pdvb_stage_value CHECK (
        (status = 'BLOCKED' AND pdvb_value IS NULL)
        OR (status <> 'BLOCKED' AND pdvb_value IS NOT NULL AND pdvb_value >= 0)
    ),
    CONSTRAINT ck_pdd_pdvb_stage_zero CHECK (status <> 'ZERO_VALID' OR pdvb_value = 0)
);

CREATE INDEX ix_pdd_pdvb_stage_scope
    ON stock_management.pdd_pdvb_publication_stage
    (publication_batch_id, scope_version_id, sucursal, codigo_articulo);

CREATE TABLE stock_management.pdd_pdvb_estimate (
    business_date date NOT NULL,
    pdvb_estimate_id bigint GENERATED ALWAYS AS IDENTITY,
    calculation_run_id bigint NOT NULL
        REFERENCES stock_management.pdd_calculation_run(calculation_run_id) ON DELETE RESTRICT,
    publication_batch_id bigint NOT NULL
        REFERENCES stock_management.pdd_pdvb_publication_batch(publication_batch_id) ON DELETE RESTRICT,
    analytical_detail_id bigint NOT NULL,
    model_version_id bigint NOT NULL
        REFERENCES stock_management.pdd_pdvb_model_version(model_version_id) ON DELETE RESTRICT,
    scope_version_id bigint NOT NULL
    ...
    CONSTRAINT uq_pdd_pdvb_estimate_run_pair UNIQUE (
    ...
    FOREIGN KEY (scope_version_id, sucursal, codigo_articulo)
        REFERENCES stock_management.pdd_distribution_scope_pair
            (scope_version_id, destination_branch, codigo_articulo)
        ON DELETE RESTRICT,
    CONSTRAINT ck_pdd_pdvb_estimate_origin CHECK (origin_cd = 41),
    CONSTRAINT ck_pdd_pdvb_estimate_values CHECK (
        (status = 'BLOCKED' AND pdvb_value IS NULL)
        OR (status <> 'BLOCKED' AND pdvb_value IS NOT NULL AND pdvb_value >= 0)
    ),
    CONSTRAINT ck_pdd_pdvb_estimate_zero CHECK (status <> 'ZERO_VALID' OR pdvb_value = 0)
) PARTITION BY RANGE (business_date);

CREATE INDEX ix_pdd_pdvb_estimate_pair_date
    ON stock_management.pdd_pdvb_estimate (codigo_articulo, sucursal, business_date DESC);

CREATE INDEX ix_pdd_pdvb_estimate_run_status
    ON stock_management.pdd_pdvb_estimate (calculation_run_id, status);

CREATE INDEX ix_pdd_pdvb_estimate_scope
    ON stock_management.pdd_pdvb_estimate (scope_version_id, origin_cd, business_date DESC);

CREATE TABLE stock_management.pdd_pdvb_current (
    origin_cd integer NOT NULL,
    codigo_articulo integer NOT NULL,
    sucursal integer NOT NULL,
    business_date date NOT NULL,
    ...
    FOREIGN KEY (scope_version_id, sucursal, codigo_articulo)
        REFERENCES stock_management.pdd_distribution_scope_pair
            (scope_version_id, destination_branch, codigo_articulo)
        ON DELETE RESTRICT,
    CONSTRAINT ck_pdd_pdvb_current_origin CHECK (origin_cd = 41)
);

CREATE INDEX ix_pdd_pdvb_current_date_status
    ON stock_management.pdd_pdvb_current (business_date, status);

CREATE INDEX ix_pdd_pdvb_current_scope
    ON stock_management.pdd_pdvb_current (scope_version_id, origin_cd, sucursal, codigo_articulo);

CREATE TABLE stock_management.pdd_pdvb_quality_issue (
    ...
    CONSTRAINT ck_pdd_pdvb_issue_resolution CHECK (resolved_at IS NULL OR resolved_by IS NOT NULL)
);

CREATE INDEX ix_pdd_pdvb_quality_open
    ON stock_management.pdd_pdvb_quality_issue (severity, business_date DESC, issue_code)
    WHERE resolved_at IS NULL;

CREATE INDEX ix_pdd_pdvb_quality_pair
    ON stock_management.pdd_pdvb_quality_issue (codigo_articulo, sucursal, business_date DESC)
    WHERE codigo_articulo IS NOT NULL AND sucursal IS NOT NULL;

CREATE TABLE stock_management.pdd_pdvb_backtest_metric (
    ...
    detail jsonb NOT NULL DEFAULT '{}'::jsonb,
    CONSTRAINT uq_pdd_pdvb_backtest_metric UNIQUE (
        calculation_run_id, evaluation_from, evaluation_to, forecast_horizon_days,
        segment_type, segment_id, metric_code
    ),
    CONSTRAINT ck_pdd_pdvb_backtest_dates CHECK (evaluation_to >= evaluation_from)
);

CREATE INDEX ix_pdd_pdvb_backtest_model_period
    ON stock_management.pdd_pdvb_backtest_metric (model_version_id, evaluation_to DESC);

COMMENT ON TABLE stock_management.pdd_distribution_scope_version IS
'Version inmutable del universo distribuible desde CD 41; los conteos se calculan y nunca se hardcodean.';

COMMENT ON CONSTRAINT ck_pdd_distribution_scope_pair_phase1_cd41
    ON stock_management.pdd_distribution_scope_pair IS
'Regla ADR-001: cod_cd=41CD, abastecimiento=0, habilitado=1, active_for_sale=1 y active_on_mix=1.';

COMMENT ON TABLE stock_management.pdd_pdvb_publication_stage IS
'Staging transaccional para publicar el subconjunto PDVB desde diarco_data sin consultar tablas pesadas por FDW.';

COMMENT ON TABLE stock_management.pdd_pdvb_estimate IS
'Historia operativa compacta de PDVB; el detalle completo permanece en diarco_data.datamart.dm_pdd_pdvb_estimate_detail.';

COMMENT ON COLUMN stock_management.pdd_pdvb_estimate.analytical_detail_id IS
'Referencia logica sin FK cross-database al detalle analitico de diarco_data.';

COMMENT ON TABLE stock_management.pdd_pdvb_current IS
