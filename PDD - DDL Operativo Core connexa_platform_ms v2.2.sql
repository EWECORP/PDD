...
CREATE TABLE stock_management.configuration_version (
...
CREATE TABLE stock_management.pdvb_model_version (
...
CREATE TABLE stock_management.distribution_scope_version (
...
CREATE UNIQUE INDEX uq_distribution_scope_current
    ON stock_management.distribution_scope_version (origin_cd)
    WHERE is_current;

CREATE TABLE stock_management.distribution_scope_article (
...
CREATE INDEX ix_distribution_scope_article_lookup
    ON stock_management.distribution_scope_article (codigo_articulo, scope_version_id DESC);

CREATE TABLE stock_management.distribution_scope_pair (
    scope_version_id bigint NOT NULL,
    origin_cd integer NOT NULL,
    ...
    FOREIGN KEY (scope_version_id, codigo_articulo)
        REFERENCES stock_management.distribution_scope_article (scope_version_id, codigo_articulo)
        ON DELETE RESTRICT,
...
CREATE INDEX ix_distribution_scope_pair_operational
    ON stock_management.distribution_scope_pair
    (scope_version_id, destination_branch, codigo_articulo, c_proveedor_primario);

CREATE TABLE stock_management.calculation_run (
    ...
    scope_version_id bigint REFERENCES stock_management.distribution_scope_version(scope_version_id),
    model_version_id bigint REFERENCES stock_management.pdvb_model_version(model_version_id),
    configuration_version_id bigint
        REFERENCES stock_management.configuration_version(configuration_version_id),
...
CREATE UNIQUE INDEX uq_calculation_run_current
    ON stock_management.calculation_run (run_type, business_date, scope_type, scope_id)
    WHERE is_current;

CREATE INDEX ix_calculation_run_status_date
    ON stock_management.calculation_run (status, business_date DESC);

CREATE TABLE stock_management.source_snapshot (
...
CREATE INDEX ix_source_snapshot_status
    ON stock_management.source_snapshot (calculation_run_id, status)
    WHERE status <> 'VALID';

CREATE TABLE stock_management.pdvb_publication_batch (
...
CREATE TABLE stock_management.pdvb_publication_stage (
    publication_batch_id bigint NOT NULL
        REFERENCES stock_management.pdvb_publication_batch(publication_batch_id) ON DELETE CASCADE,
    business_date date NOT NULL,
    analytical_detail_id bigint NOT NULL,
    model_version_id bigint NOT NULL
        REFERENCES stock_management.pdvb_model_version(model_version_id) ON DELETE RESTRICT,
    scope_version_id bigint NOT NULL
        REFERENCES stock_management.distribution_scope_version(scope_version_id) ON DELETE RESTRICT,
    origin_cd integer NOT NULL,
    codigo_articulo integer NOT NULL,
...
CREATE INDEX ix_pdvb_stage_scope
    ON stock_management.pdvb_publication_stage
    (publication_batch_id, scope_version_id, sucursal, codigo_articulo);

CREATE TABLE stock_management.pdvb_estimate (
    business_date date NOT NULL,
    pdvb_estimate_id bigint GENERATED ALWAYS AS IDENTITY,
    calculation_run_id bigint NOT NULL
        REFERENCES stock_management.calculation_run(calculation_run_id) ON DELETE RESTRICT,
    publication_batch_id bigint NOT NULL
        REFERENCES stock_management.pdvb_publication_batch(publication_batch_id) ON DELETE RESTRICT,
    analytical_detail_id bigint NOT NULL,
    model_version_id bigint NOT NULL
        REFERENCES stock_management.pdvb_model_version(model_version_id) ON DELETE RESTRICT,
    scope_version_id bigint NOT NULL
...
CREATE INDEX ix_pdvb_estimate_pair_date
    ON stock_management.pdvb_estimate (codigo_articulo, sucursal, business_date DESC);

CREATE INDEX ix_pdvb_estimate_run_status
    ON stock_management.pdvb_estimate (calculation_run_id, status);

CREATE INDEX ix_pdvb_estimate_scope
    ON stock_management.pdvb_estimate (scope_version_id, origin_cd, business_date DESC);

CREATE TABLE stock_management.pdvb_current (
    origin_cd integer NOT NULL,
    codigo_articulo integer NOT NULL,
    sucursal integer NOT NULL,
    business_date date NOT NULL,
...
CREATE INDEX ix_pdvb_current_date_status
    ON stock_management.pdvb_current (business_date, status);

CREATE INDEX ix_pdvb_current_scope
    ON stock_management.pdvb_current (scope_version_id, origin_cd, sucursal, codigo_articulo);

CREATE TABLE stock_management.pdvb_quality_issue (
...
CREATE INDEX ix_pdvb_quality_open
    ON stock_management.pdvb_quality_issue (severity, business_date DESC, issue_code)
    WHERE resolved_at IS NULL;

CREATE INDEX ix_pdvb_quality_pair
    ON stock_management.pdvb_quality_issue (codigo_articulo, sucursal, business_date DESC)
    WHERE codigo_articulo IS NOT NULL AND sucursal IS NOT NULL;

CREATE TABLE stock_management.pdvb_backtest_metric (
...
CREATE INDEX ix_pdvb_backtest_model_period
    ON stock_management.pdvb_backtest_metric (model_version_id, evaluation_to DESC);

COMMENT ON TABLE stock_management.distribution_scope_version IS
