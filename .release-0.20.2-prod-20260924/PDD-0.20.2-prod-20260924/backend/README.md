...
Para la promoción inicial de PROD de 2026-09-23 se usa:

- scope `c533e7e7-3363-4dc3-8127-24f48fe33522`;
- modelo `a0a35b25-628d-43f1-b651-82c97207fc60`;
- configuración PROD `3e5bd515-8642-4eb7-8058-844c67beb408`;
- pipeline `DAILY_PIPELINE_V3`.

El scope se promueve de forma transaccional y auditable mediante
`PDD - Promocion Scope PROD 20260924.sql`. Los manifiestos aprobados incluyen
`approved_by`; al materializarlos por primera vez en la base operativa, PDD
registra además `approved_at`. Una versión `APPROVED` sin esos metadatos se
...
Producción no se despliega con ese archivo. `prefect.prod.yaml` contiene
exclusivamente deployments con sufijo `_PROD`, usa la cola existente `pdd` y
entrega inicialmente inactivos los schedules. Esto evita que TEST, DESA y PROD
se sobrescriban por compartir nombres dentro del mismo Prefect Server.

Desde 0.20.2, `DAILY_DECAS` aplica también la elegibilidad capturada desde
`inventory.inv_planning_parameters_v`: un par con `product_site_active=false`
o `active_for_purchase=false` conserva posición y necesidades auditables con
estado `BLOCKED`, cantidades nulas y alertas explícitas, pero nunca contribuye
al backlog. Las necesidades dirigidas E/C/A activas sobre uno de esos pares
bloquean la publicación para exigir su regularización.
...
Para Producción, después de validar el artefacto y durante una ventana de
mantenimiento:

```bash
export PREFECT_API_URL=https://orquestador.connexa-cloud.com/api
prefect deploy --prefect-file prefect.prod.yaml --all
```

Antes de ejecutar este comando se debe confirmar su sintaxis mediante
`prefect deploy --help` en la versión instalada. La primera corrida productiva
se inicia manualmente; el schedule diario se activa sólo después de validar su
resultado.
...
El archivo `.env.desa.example` se usa solamente como plantilla. En el servidor
se instala como `/etc/connexa/pdd-desa.env`, propiedad `root:pdd`, modo `0640`;
no se coloca dentro del código ni se versiona. El worker DESA debe ser
independiente y usar el virtualenv propio de PDD:

```text
Environment="PDD_ENV_PATH=/etc/connexa/pdd-desa.env"
ExecStart=/srv/PDD/.venv/bin/prefect worker start \
...
Para DESA se instala desde la plantilla `.env.desa.example`: host `186.158.182.122`, base
