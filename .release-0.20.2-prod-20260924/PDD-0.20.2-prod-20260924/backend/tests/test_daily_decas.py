...
def test_inventory_ineligible_pair_is_blocked_without_open_needs() -> None:
    branch = build_branch_position(
        _source(
            planning_input={
                "product_site_active": False,
    ...
    assert branch["calculation_status"] == "BLOCKED"
    assert "INVENTORY_PRODUCT_SITE_INACTIVE" in branch["alert_codes"]
    assert "INVENTORY_PURCHASE_INACTIVE" in branch["alert_codes"]
    assert len(needs) == 2
    assert all(row["calculation_status"] == "BLOCKED" for row in needs)
    assert all(row["calculated_quantity"] is None for row in needs)
    assert all(row["rounded_quantity"] is None for row in needs)
    assert all(row["open_quantity"] is None for row in needs)
