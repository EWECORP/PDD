...
def build_branch_position(
    ...
    inventory_ineligible = planning_input is not None and (
        planning_input.get("product_site_active") is not True
        or planning_input.get("active_for_purchase") is not True
    )
    if planning_input is not None:
        if planning_input.get("product_site_active") is not True:
            alerts.append("INVENTORY_PRODUCT_SITE_INACTIVE")
        if planning_input.get("active_for_purchase") is not True:
            alerts.append("INVENTORY_PURCHASE_INACTIVE")

    if inventory_ineligible:
        status = "BLOCKED"
    elif pdvb == 0:
        status = "ZERO_PDVB"
    elif alerts:
...
def build_need_rows(
    ...
    if branch["calculation_status"] == "BLOCKED":
        result: list[dict[str, Any]] = []
        for need_type, mandatory in (("D", True), ("S", False)):
            row = {
                ...
                "estimated_packages": None,
                "estimated_pallets": None,
                "estimated_weight_kg": None,
                "estimated_volume_m3": None,
                "calculation_status": "BLOCKED",
                "alert_codes": list(branch["alert_codes"]),
                "explanation": {
                    "blocked_by": "INVENTORY_PURCHASE_ELIGIBILITY",
...
def _ensure_configuration(
    ...
    existing = connection.execute(
        text(
            ...
            SELECT configuration_version_id, configuration_code, version_no,
                   status, valid_from, parameters, checksum, approved_by
    ...
    if existing is not None:
        if (
            ...
            or existing["valid_from"] != configuration.valid_from
            or existing["parameters"] != configuration.parameters
            or existing["checksum"].strip() != configuration.checksum
            or existing["approved_by"] != configuration.approved_by
        ):
...
def run_daily_decas(
    ...
    with transactional_connection(target_engine, target_settings) as target:
        ...
        summary = {
            "entity": configuration.configuration_code,
