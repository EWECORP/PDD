...
def _ensure_model(
    ...
    connection.execute(
        text(
            """
            INSERT INTO stock_management.pdd_pdvb_model_version AS stored_model (
                model_version_uuid, model_code, version_no, status,
                valid_from, parameters, implementation_sha256, code_commit_sha,
                created_by, approved_at, approved_by
            ) VALUES (
                :model_version_uuid, :model_code, :version_no, :status,
                :valid_from, CAST(:parameters AS jsonb), :implementation_sha256,
                :code_commit_sha, :created_by,
                CASE WHEN :status = 'APPROVED' THEN clock_timestamp() END,
                :approved_by
            )
            ON CONFLICT (model_version_uuid) DO UPDATE
            SET status = EXCLUDED.status,
                valid_from = EXCLUDED.valid_from,
                approved_at = CASE
                    WHEN stored_model.approved_at IS NULL
                    THEN clock_timestamp()
                    ELSE stored_model.approved_at
                END,
                approved_by = EXCLUDED.approved_by
            WHERE stored_model.status = 'DRAFT'
              AND EXCLUDED.status = 'APPROVED'
              AND stored_model.model_code = EXCLUDED.model_code
              AND stored_model.version_no = EXCLUDED.version_no
              AND stored_model.parameters = EXCLUDED.parameters
              AND btrim(stored_model.implementation_sha256)
                  = btrim(EXCLUDED.implementation_sha256)
              AND stored_model.code_commit_sha IS NOT DISTINCT FROM
                  EXCLUDED.code_commit_sha
            """
        ),
        {
            **manifest.__dict__,
    ...
    stored = connection.execute(
        text(
            """
            SELECT model_version_id, model_code, version_no, status,
                   valid_from, parameters, implementation_sha256, approved_by
...
def _ensure_scope(
    ...
    if scope["status"] == "APPROVED" and not (approved_at and approved_by):
        raise RuntimeError(
            "El scope APPROVED carece de approval.approved_at/approved_by"
        )
    connection.execute(
        text(
            """
            INSERT INTO stock_management.pdd_distribution_scope_version (
                ...
                destination_count, checksum, created_at, created_by,
                approved_at, approved_by, detail
            ) VALUES (
                ...
                :created_at, :created_by, CAST(:approved_at AS timestamptz),
                :approved_by, CAST(:detail AS jsonb)
            )
    ...
    stored = connection.execute(
        text(
            """
            SELECT scope_version_id, status, article_count, pair_count,
                   checksum, approved_at, approved_by
    ...
    if (
        stored["status"] != scope["status"]
        or stored["article_count"] != scope["article_count"]
        or stored["pair_count"] != scope["pair_count"]
        or stored["checksum"].strip() != scope["scope_checksum"].strip()
        or stored["approved_by"] != approved_by
    ):
